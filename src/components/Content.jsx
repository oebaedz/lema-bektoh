...
const Content = () => {
    ...
    const url = `https://api.aladhan.com/v1/timingsByAddress?address=${city}`
    ...
    return (
        <div className='flex flex-col md:px-64'>
            <div className='flex flex-col gap-4 my-4 text-center items-center justify-center md:justify-between md:flex-row'>
                ...
                <div>
                    <p className="font-medium text-lg">{data.date && data.date.hijri.weekday.ar}</p>
                    <p className="font-light text-md">{data.date && `${ConvertToArabicNumbers(data.date.hijri.day)} ${data.date.hijri.month.ar} ${ConvertToArabicNumbers(data.date.hijri.year)}`}</p>
                    <p className="font-light text-md">{data.date && data.date.readable}</p>
                </div>
            </div>
            <form onSubmit={(e) => {
                e.preventDefault()
